...
Page({
  ...
  jump1: function () {
    ...
    if (that.data.phase_index != 0) {
      ...
      var info = that.data.phase[that.data.phase_index] + "," + date + "," + that.data.ranchid[that.data.ranch_index] + "," + that.data.tanklist[that.data.tank_index] + "," + that.data.ranchlist[that.data.ranch_index]
      wx.navigateTo({
        url: '../historyinfo/historyinfo?info=' + info
      })
    } else {
      wx.showModal({
        title: '错误',
